  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <div id="invoice" className="bg-white rounded-xl shadow-sm border border-gray-200 p-6" dir="rtl">
        ...
        {/* Header - hidden on print (pre-printed on paper) */}
        <div className="print-hide-header text-center border-b border-gray-200 pb-4 mb-4">
          <h1 className="text-2xl font-bold">{sale.branch?.name}</h1>
          <p className="text-gray-500 mt-1">{sale.branch?.address}</p>
          {sale.branch?.vat_number && (
            <p className="text-gray-400 text-sm" dir="ltr">VAT: {sale.branch.vat_number}</p>
          )}
        </div>

        {/* Title row */}
        <div className="flex items-center border-t border-l border-r border-black mb-0">
          <div className="flex-1 p-2 border-l border-black">
            <p className="text-xs text-gray-600">فاتورة ضريبية مبسطة</p>
        ...
        <table className="w-full border-collapse text-sm mt-3">
          <thead>
            <tr className="bg-gray-100">
              <th className="border border-black p-1 text-center">م</th>
              <th className="border border-black p-1 text-center">باركود</th>
              <th className="border border-black p-1 text-right">اسم الصنف</th>
              <th className="border border-black p-1 text-center">الوحدة</th>
              <th className="border border-black p-1 text-center">العدد</th>
              <th className="border border-black p-1 text-center">السعر</th>
              <th className="border border-black p-1 text-center">الإجمالي</th>
            </tr>
          </thead>
          <tbody>
            {sale.items?.map((item, i) => (
              <tr key={item.id}>
                ...
                <td className="border border-black p-1 text-center">{item.product?.unit || 'قطعة'}</td>
                <td className="border border-black p-1 text-center">{item.quantity}</td>
                <td className="border border-black p-1 text-center">{Number(item.unit_price).toFixed(2)}</td>
                <td className="border border-black p-1 text-center font-bold">{Number(item.total).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="totals-table border-collapse text-sm mt-3 w-3/5">
          <tbody>
            ...
            <tr>
              <td className="border border-black p-1 font-bold">الإجمالي قبل الضريبة :</td>
              <td className="border border-black p-1 text-center">{subtotal.toFixed(2)}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td className="border border-black p-1 font-black text-base">الإجمالي شامل الضريبة :</td>
              <td className="border border-black p-1 text-center font-black text-base">{total.toFixed(2)}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td className="border border-black p-1 font-bold">المتبقي :</td>
              <td className="border border-black p-1 text-center">{remaining.toFixed(2)}</td>
            </tr>
          </tbody>
        </table>
